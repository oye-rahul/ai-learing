...
const PythonEnvironment: React.FC<PythonEnvironmentProps> = ({ environment, onBack }) => {
...
`);
  ...
  const handleExplainCode = async () => {
    ...
    try {
      const response = await aiAPI.explainCode({ code, language: 'python' });
  ...
  const handleFixCode = async () => {
    ...
    try {
      const response = await aiAPI.debugCode({ code, language: 'python' });
